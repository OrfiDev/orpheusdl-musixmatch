...
class ModuleInterface:
    def __init__(self, module_controller: ModuleController):
        ...
        self.musixmatch.user_token = module_controller.temporary_settings_controller.read('user_token', 'global')
        if not self.musixmatch.user_token:
            self.musixmatch.user_token = self.musixmatch.get_user_token()
            module_controller.temporary_settings_controller.set('user_token', self.musixmatch.user_token, 'global')
    ...
    def search(self, querytype, query, track_info: TrackInfo = None):
        ...
        if track_info and track_info.tags.isrc:
            ...
            if track:
                ...
                lyrics = self.musixmatch.get_subtitle_by_id(track_id)
        if not track:
    ...
    def get_track_lyrics(self, lyrics_id, lyrics=None) -> Optional[LyricsInfo]:
        return LyricsInfo(
